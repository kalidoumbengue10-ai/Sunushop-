...
export async function POST(
  ...
  try {
    ...
    if (order) await enqueueEmail(admin, { dedupeKey: `order-status:${id}:${input.status}`, template: "order_status_changed", recipientUserId: order.buyer_id, payload: { orderCode: order.public_code, statusLabel: merchantStatusLabel(input.status), publicMessage: input.publicMessage, url: new URL(`/commandes/${id}`, request.url).toString() } }).catch(() => false);
