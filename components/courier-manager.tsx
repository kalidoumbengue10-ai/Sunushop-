...
export function CourierManager({ merchantId, canManagePayments = false }: { merchantId: string; canManagePayments?: boolean }) {
  ...
  const load = useCallback(async () => {
    const [courierResponse, deliveryResponse, paymentResponse, assignableResponse] = await Promise.all([
    ...
    const [courierPayload, deliveryPayload, paymentPayload, assignablePayload] = await Promise.all([courierResponse.json(), deliveryResponse.json(), paymentResponse.json(), assignableResponse.json()]);
    ...
    if (!assignableResponse.ok) throw new Error(assignablePayload.error?.message);
    ...
    setAssignable(assignablePayload.data);
  }, [merchantId]);
  ...
  const invite = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const ok = await submit("/api/merchant/couriers", "POST", JSON.stringify({ merchantId, email: values.get("email"), displayName: values.get("displayName"), phone: values.get("phone"), vehicleType: values.get("vehicleType") || undefined, vehicleRegistration: values.get("vehicleRegistration") || undefined }), "Livreur enregistré. Un lien d’activation lui a été envoyé par email.");
    if (ok) form.reset();
  };
