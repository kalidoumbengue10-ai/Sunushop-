...
export function CourierManager({ merchantId, orders }: { merchantId: string; orders: Array<{ id: string; public_code: string; status: string }> }) {
  ...
  return <div className="courier-manager">
    ...
    <section className="merchant-kpi-grid"><article><span>Missions actives</span><strong>{stats.active}</strong></article><article><span>Livrées ce mois</span><strong>{stats.deliveredThisMonth}</strong></article><article><span>Frais livrés</span><strong>{formatPrice(stats.grossDeliveryFeesXof)}</strong></article><article><span>Commission SunuShop</span><strong>{formatPrice(stats.platformCommissionXof)}</strong><small>Inactive</small></article></section>
