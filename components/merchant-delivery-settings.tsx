...
export function MerchantDeliverySettings({ merchantId, categories, zones, pickupSettingEnabled }: { merchantId: string; categories: Array<{ id: string; name: string }>; zones: MerchantDeliveryZone[]; pickupSettingEnabled?: boolean }) {
  ...
  const [courierFeeXof, setCourierFeeXof] = useState("");
  ...
  const loadZone = (zone: MerchantDeliveryZone) => {
    ...
    setRegion(zone.region); setFeeXof(zone.fee_xof); setCourierFeeXof(zone.courier_fee_xof == null ? "" : String(zone.courier_fee_xof)); setMinDays(Math.floor(zone.min_delay_minutes / 1440)); setMaxDays(Math.ceil(zone.max_delay_minutes / 1440));
  ...
  const save = async (event: FormEvent) => {
    ...
    const response = await fetch("/api/merchant/delivery-regions", { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify({ merchantId, zoneId: editingZoneId, region, enabled: true, feeXof, courierFeeXof: Number(courierFeeXof), minDelayDays: minDays, maxDelayDays: maxDays, categoryRates: Object.entries(rates).filter(([, fee]) => fee !== "").map(([categoryId, fee]) => ({ categoryId, feeXof: Number(fee) })) }) });
  ...
  return <div className="merchant-delivery-space"><div className="merchant-section-heading"><div><span className="mvp-eyebrow">Livraison</span><h2>Où livrez-vous ?</h2><p>Un tarif de base par région, puis seulement les exceptions utiles par catégorie.</p></div></div>{message && <p className="mvp-alert" role="status">{message}</p>}{error && <p className="mvp-alert mvp-alert--error" role="alert">{error}</p>}
    ...
    <form className="mvp-form delivery-region-form" onSubmit={save}><h3>Ajouter ou modifier une région</h3><div className="mvp-form__grid"><label className="mvp-field">Région<select value={region} onChange={(event) => setRegion(event.target.value)}>{SENEGAL_REGIONS.map((name) => <option value={name} key={name}>{name}</option>)}</select></label><label className="mvp-field">Prix facturé au client (FCFA)<input type="number" min="0" value={feeXof} onChange={(event) => setFeeXof(Number(event.target.value))} required /></label><label className="mvp-field">Rémunération du livreur (FCFA)<input type="number" min="0" value={courierFeeXof} onChange={(event) => setCourierFeeXof(event.target.value)} required /><small>Ce montant sera figé pour chaque mission affectée dans cette zone.</small></label><label className="mvp-field">Livraison au plus tôt (jours)<input type="number" min="0" max="30" value={minDays} onChange={(event) => setMinDays(Number(event.target.value))} required /></label><label className="mvp-field">Livraison au plus tard (jours)<input type="number" min={minDays} max="30" value={maxDays} onChange={(event) => setMaxDays(Number(event.target.value))} required /></label></div><details className="delivery-category-overrides"><summary>Ajouter des tarifs particuliers par catégorie</summary><p>Une case vide conserve le tarif régional. Pour un panier mixte, SunuShop retient le tarif le plus élevé.</p><div className="mvp-form__grid">{categories.map((category) => <label className="mvp-field" key={category.id}>{category.name}<input type="number" min="0" placeholder={`${feeXof} par défaut`} value={rates[category.id] ?? ""} onChange={(event) => setRates((current) => ({ ...current, [category.id]: event.target.value }))} /></label>)}</div></details><button className="mvp-button" disabled={busy}>{busy ? "Enregistrement…" : `Enregistrer ${region}`}</button></form>
    <div className="delivery-region-grid">{deliveryZones.map((zone) => <article key={zone.id}><span className="mvp-eyebrow">Région desservie</span><h3>{zone.region}</h3><strong>{formatPrice(zone.fee_xof)} client</strong><p>{zone.courier_fee_xof == null ? "Rémunération livreur à configurer" : `${formatPrice(zone.courier_fee_xof)} pour le livreur`} · {Math.floor(zone.min_delay_minutes / 1440)} à {Math.ceil(zone.max_delay_minutes / 1440)} jour(s)</p><button className="mvp-button mvp-button--secondary" onClick={() => loadZone(zone)}>Modifier les tarifs</button></article>)}</div>
  </div>;
}
